...
A no-change refresh records a new observation boundary and associates it with the active semantic publication, but it does not create a new semantic publication revision or update public rows. The original publication manifest remains immutable; `mdm.describe()` distinguishes the frontier at which the revision was first published from the later frontier through which the same result has been proven equivalent. Different entities may refresh concurrently. Only one refresh, rebuild, or activation for a particular entity may execute at a time.
