...
> [!IMPORTANT]
> v0.4 adds complete, bounded candidate generation and graph-stage compilation. It does not execute a live graph, resolve records, or create public output tables.
...
## Install v0.4 (developmental definitions, normalization, and candidates)

v0.4 supports PostgreSQL 18 and requires `pg_trickle` 0.98.0. Add `pg_trickle` to `shared_preload_libraries`, restart PostgreSQL, and install `pg_trickle` first. [`DEPENDENCIES.md`](DEPENDENCIES.md) records the release artifact and image digests.

The v0.4 actions store and validate definitions and compile record, normalization, and candidate stages. They do not execute graph SQL, resolve records, or create public output tables.
