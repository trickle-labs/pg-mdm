...
> [!IMPORTANT]
> v0.5 adds bounded evidence evaluation, deterministic pair decisions, and durable steward decisions. The graph remains non-executable; clustering, publication, and review persistence are later releases.
...
## Install v0.5 (evidence, pair decisions, and stewardship)

v0.5 supports PostgreSQL 18 and requires `pg_trickle` 0.98.0. Add `pg_trickle` to `shared_preload_libraries`, restart PostgreSQL, and install `pg_trickle` first. [`DEPENDENCIES.md`](DEPENDENCIES.md) records the release artifact and image digests.

The v0.5 actions store and validate definitions, compile record, normalization, candidate, and evidence stages, and accept explicit steward decisions. They do not execute graph SQL, resolve records, or create public output tables.
