...
> [!IMPORTANT]
> v0.14 requires `pg_trickle` 0.108.0. It consumes Delta V1.1 terminal changes and adds bound stewardship policy intents with durable receipts.
...
1. Upgrade `pg_trickle` to 0.108.0, then upgrade `pg_mdm` to 0.14.0.
...
The v0.14 release admits the checksummed `pg_trickle` 0.108.0 artifact. It adds policy-intent bindings, atomic control updates, immutable receipts, and fail-closed retry behavior on top of v0.13's differential graph refresh. The CI suite checks full-result equivalence and the documented recovery paths.
